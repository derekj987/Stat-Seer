...
// heavy favorites cover ~half the time and our rating doesn't beat the spread — so we show NO tag at
// all (rather than an underdog lean). On closer games we show the real line-blind lean (which side
// our number has covering) with the margin.
const BIG_SPREAD = 17;
...
export function ncaafLean(g: NcaafCardGame): { cls: string; txt: string; tip: string } | null {
  ...
  if (line >= BIG_SPREAD) return null;   // big favorite: market is efficient — show no tag (no lean)
