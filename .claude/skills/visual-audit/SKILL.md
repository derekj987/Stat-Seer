...
### 🚨 A pipeline whose INPUTS are gitignored fails silently — and can revert a correct number
The highest-cost bug of 2026-09-25, and it had been live for days. `injury_adj.load_usage()` reads
`data/stats_<season>.csv` to work out what share of his team's workload an absent player held.
`data/` is gitignored, `publish-predictions.yml` never fetched it, and `load_usage()` **returns
`None`** when the files are missing. `team_adjustment()` then returns `{}` and the caller "predicts
unadjusted" — with no error, no warning, and **no way to tell that answer apart from "nobody is
hurt."** Every scheduled run published injury-blind.

It did not merely go stale. It **actively reverted a correct number**: NYG published at +6.0, was
hand-corrected to +3.1 after Jaxson Dart went on IR, and the next scheduled run recomputed +6.0 with
no adjustment, saw a 2.9-point gap, and "revised" the good number back to the bad one.

**Two rules came out of it:**
- **A function that answers "no data" and "nothing to report" with the same empty value is a
  landmine on any live path.** It is correct for a backtest (a season with no usage file must
  predict unadjusted rather than crash) and wrong for publishing. Check the INPUT explicitly before
  drawing any conclusion from the output.
- **Grep every workflow for the files its scripts read.** `data/` is regenerated by
  `analysis/fetch_data.py`; `player-projections.yml` fetches the stats CSVs with `curl` and
  `publish-predictions.yml` did not. The two ran the same library.

```bash
grep -rn "data/stats_\|data/inj_\|data/roster_" --include=*.py . | cut -d: -f1 | sort -u
grep -rLn "curl .*nflverse" .github/workflows/*.yml      # workflows that fetch nothing
```

### 🚨 A check that recomputes its subject IN THE SAME PROCESS is an echo
This has now bitten twice, in two different guardrails, so it gets the strongest possible statement.

`injury_guardrail` check B compared the live model's margin against the published margin. On the
runner, with the usage CSVs missing, **both sides went blind together and agreed** — it printed
*"every unplayed game's published margin is within 0.75 of the live model"* while the site served
NYG at +6.0 with a quarterback on injured reserve. The earlier version of check A had the same shape
(it rebuilt its out-list from `injury_adj`'s own helpers, so disabling the news merge blinded the
model and the check at the same instant).

**A comparison between two copies of the same broken thing proves nothing.** Either read the
independent source directly (`sleeper_availability_current` and `practice_reports` straight from the
database; `statsapi` rather than `mlb_availability`; CFBD rather than `cfb_export`), or — when the
check genuinely must recompute — have it **refuse to certify** when its inputs are missing:

```python
data_ok = check_news_to_adjustment(...)          # returns False when usage data is absent
check_published_is_current(..., data_ok=data_ok) # warns instead of passing
```

### An append-on-change table's newest row is the last NEWS, not the last RUN
`sleeper_availability` is written only when something changes — the capture's own log line for a
quiet run is `nothing to write`. A freshness check that failed when the newest row was older than
12h **failed a perfectly healthy feed**: `capture-sleeper` ran at 13:36, wrote nothing because
nothing had changed, and the guardrail went red at 18:09.

**A check that goes red on a healthy system is worse than no check, because it teaches you to ignore
the red.** The table cannot answer "did the job run" — it has no heartbeat row — so the check must
not pretend to ask that. It is now a WARN worded as what it measures ("newest availability CHANGE is
14.1h old"). If you need true liveness, add a heartbeat row; do not infer it from content.

### 🚨 `offset=` silently stops paging at 10,000 — `Range` headers are the only correct pager
A second, nastier face of the 1000-row cap already documented above. Paging with
`&limit=1000&offset=N` **stops returning rows past offset 10,000** on this Supabase instance, with
no error — the loop sees a short page and concludes it has finished.

Measured: `mlb_odds_snapshots` holds **30,540** totals rows; an `offset=` loop read **10,000** and
looked complete. The analysis built on it reported the market 1.52 runs below actual scoring — a
number that should have been the tell, because books are never 1.5 runs off. Paged properly with
`Range` headers the real figure was **0.74**.

```python
# WRONG — stops at 10,000 without saying so
f"...&limit=1000&offset={off}"
# RIGHT — pg() in analysis/player_proj_export.py
headers={"Range": f"{off}-{off+999}", "Range-Unit": "items"}
```

Two tells that a read truncated: a total that is a round multiple of 1000, and a result too large to
be plausible. **When a measurement says the market is badly wrong, suspect the measurement.**

### 🚨 Two percentages on one row must describe the SAME side
Derek: *"our spreads are suggesting to take the +1.5 for every team. This is not correct."*

The MLB board's market cell printed the de-vigged chance the **market's favourite** covers −1.5. The
cell beside it printed our chance for whichever side **we** rated higher — almost always the
underdog, because favourites cover −1.5 in only 39% of games. So a row read `fair 39%` next to
`BOS +1.5 62%`, and comparing 39 with 62 looks like an enormous edge. **The two numbers were about
opposite teams.** Converted to the same side the market implied 61% against our 62% — no
disagreement at all, on 8 of 11 games our number was *below* the market's.

Keying the choice on OUR favourite also **named sides that are not posted**: "PIT +1.5" while the
book had PIT at −1.5. A run line has exactly two sides and both belong to the market.

**Whenever a board shows "ours" beside "theirs", assert they refer to the same side, at the same
number, in the same direction.** Anchor the row to the market's posted sides and derive ours to
match — never the reverse.

### A model panel must not print an imperative
The same cell said **"take the run" / "lay the run"** while the scroll above it said *"a gap here is
a difference to notice, not an edge to act on"*. The panel contradicted its own caveat, in verbs.
Panels inform; they do not vote. Print the side, our number, and the market's number for that side —
and let the reader decide. Grep for instruction words in model surfaces:

```bash
grep -rniE ">(take|lay|back|fade|bet|play) " web/app --include=*.tsx
```

### Day + matchup is not a unique key — doubleheaders exist
`new Map(board.map(g => [`${etDayKey(g.commence)}|${g.matchup}`, g]))` silently kept whichever game
arrived last, so **both** games of a doubleheader rendered **one** game's run line and total. Live
on 2026-09-25 (BAL @ NYY, games 1 and 2).

The fix is nearest first pitch — but the two clocks disagree by more than you would guess.
**statsapi lists game 2 at a placeholder time five minutes after game 1** (it starts when game 1
ends); the books post a real estimate ~3h later. So the pairing must tolerate a large disagreement
while never crossing games: **greedy one-to-one on the smallest gap**, both sides consumed, and a
game with no candidate left keeps its dash rather than borrowing another game's price.

### Level vs shape: a near-constant projection beside a varying market
Derek: *"Not every MLB game will be an over. Period."* He was right, and the projection was not the
cause. The decomposition that settled it — run this before concluding anything about a one-sided
board:

| | mean | sd | |
|---|---|---|---|
| Actual games (2,387 completed) | **8.95** | — | ground truth |
| Our projection | **8.91** | 0.94 | level correct |
| Market totals in the feed | **8.15** | 0.95 | 0.74 low |

Our level was right to within 0.04; the feed's was offset. **A constant offset applied to every game
reads as N independent findings, and a reader cannot tell the difference.** Three separate questions,
always asked in this order:
1. **Level** — does our mean match reality? (Compare to actual outcomes, not to the market.)
2. **Shape** — does our number discriminate between games? (Correlation with outcomes; MAE against
   a flat league-mean guess. Ours: +0.8%. The market's, level-corrected: +3.2% — *better than ours*.)
3. **Units** — are the two columns the same quantity? (The mean-vs-median trap, documented above.)

A board that fails (1) or (2) must not present the gap as a per-game read. `/mlb/model` now measures
its own gap at render time and says so when it is systematic — self-cancelling, so there is nothing
to remember to remove when the data agrees again.

### Measure on the population that can exhibit the outcome
Two findings on one day, both from the same mistake.

**Doubtful vs Questionable.** A first pass over all injury designations put *Questionable* at 83%
"did not play" — nonsense. The proxy for playing was a carry/target/attempt, which a lineman or
defender can never record, so every one of them scored as absent. Restricted to players with 3+
touch weeks that season:

| status | n | did not play |
|---|---|---|
| Out | 2,674 | 100.0% |
| **Doubtful** | 468 | **99.1%** |
| Questionable | 3,951 | 39.9% |

**Priced players with no projection row.** 171 of 1,230 (13.9%) looked like a coverage failure — but
155 of them were `"Alabama Crimson Tide D/ST"` and friends, team markets that correctly have no
...
| sport | what actually fails | file |
|---|---|---|
| NFL | a feed knows a starter is out; the injury adjustment does not merge him | `analysis/injury_guardrail.py` |
| MLB | a starting pitcher announced or swapped after the daily regeneration | `analysis/mlb_guardrail.py` |
| NCAAF | **nothing knows anything** — college has no injury report, so coverage and staleness | `analysis/ncaaf_guardrail.py` |

Two wiring rules that are easy to get backwards:
- **Run the guardrail AFTER the commit**, not before. Gating the commit on a red check leaves the
  *staler* board live exactly when something is wrong; the rebuilt board should ship and then the
  job should go red.
- **A git-history diff needs `fetch-depth: 0`.** `daily_digest` diffs the two most recent commits of
  the generated MLB and NCAAF boards, because those have no append-only table and **git is their
  change log**. The default shallow checkout leaves one sha reachable, the diff finds nothing, and
  the section vanishes without a word. Say so out loud when it happens rather than returning empty.

### Every workflow failing in ~3 seconds is a billing block, not your code
All jobs red across six unrelated workflows, including the repo's own CI ("failed in 3 seconds"),
with **no steps executed and no logs** (`BlobNotFound` from the logs endpoint). That is GitHub
Actions quota exhaustion, not a regression — 3 seconds is not enough to check out the repo.

Confirm before debugging code: the account's Actions usage page, and a run object whose job has an
empty `steps` array. Scheduled load is worth knowing in advance — **123 runs/day** here, and GitHub
bills a **one-minute minimum per job, rounded up**, so short frequent captures are the expensive
pattern (`capture-cfb-odds` at every 30 minutes was 39% of all runs).

```bash
python - <<'EOF'   # runs/day implied by every cron in the repo
import re, glob, os
for p in sorted(glob.glob(".github/workflows/*.yml")):
    for c in re.findall(r'cron:\s*"([^"]+)"', open(p, encoding="utf-8").read()):
        mi, hr = c.split()[0], c.split()[1]
        n = 24 // int(hr[2:]) if hr.startswith("*/") else (24 if hr == "*" else len(hr.split(",")))
        if mi.startswith("*/"): n *= 60 // int(mi[2:])
        print(f"{os.path.basename(p):38s} {n:4d}/day  {c}")
EOF
```
