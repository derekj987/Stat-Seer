...
def load_pbp():
    keep = {"season", "week", "posteam", "defteam", "play_type", "down", "ydstogo", "yardline_100",
...
def def_yards_by_coach(pbp):
    """Per-coach DEFENSIVE yards allowed per game — yards the opponent (posteam) gained while
    this coach's team was on defense (defteam), summed per game then averaged. `dcoach` is the
    defense's coach (defteam -> coach)."""
    d = pbp[pbp["play_type"].isin(["run", "pass"]) & pbp["dcoach"].notna()].copy()
    per_game = d.groupby(["dcoach", "season", "game_id"])["yards_gained"].sum().reset_index()
    career = per_game.groupby("dcoach")["yards_gained"].agg(yds="mean", g="count")
    prior = per_game[per_game.season == PRIOR_SEASON].groupby("dcoach")["yards_gained"].agg(yds="mean", g="count")
    return career, prior
...
def main():
    ...
    pbp["dcoach"] = [per_game.get((int(s), int(w), t)) if pd.notna(t) else None
                     for s, w, t in zip(pbp.season, pbp.week, pbp.defteam)]
    pbp = pbp[pbp["coach"].notna()].copy()

    # --- production: points + offensive yards + defensive yards allowed, per game ---
    yds_career, yds_prior = yards_by_coach(pbp)
    dyds_career, dyds_prior = def_yards_by_coach(pbp)
    ...
    for team, coach in sorted(current.items()):
        ...
        dyc, _ = prod(dyds_career, coach, "yds")
        dyp, _ = prod(dyds_prior, coach, "yds")
        pc, _ = prod(pts_career, coach, "pts")
        pp, _ = prod(pts_prior, coach, "pts")
        e = {"team": team, "coach": coach}
        # production (the current card): points + offensive yards + defensive yards allowed
        e.update({
            "ydsCareer": yc, "ydsPrior": yp, "ptsCareer": pc, "ptsPrior": pp,
            "defYdsCareer": dyc, "defYdsPrior": dyp,
    ...
    ts = (
        ...
        "// Shown now: production — points/game, offensive yards/game, and DEFENSIVE yards\n"
        "// allowed/game, over the coach's tenure in our play-by-play (2021-24). *Prior fields\n"
        "// (2024) are carried too. null = under a half-season of games with us. Aggressiveness\n"
        "// + pass tendency (PROE) are also carried; `rated:false` = new coach / no history.\n"
        ...
        "  defYdsCareer?: number | null; defYdsPrior?: number | null;\n"
